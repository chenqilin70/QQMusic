...
/**
 * * 根据实体类得到对应的表名、主键名、字段名（与Spring集成） * 这里使用xml文件配置的映射，需要保证实体类名与对应映射文件名一致
 * ，即User.java与User.hbm.xml* </p> * 这里使用继承Application
 * ContextAware的方式来获得ApplicationContext, * 因此需
 * 要在Spring配置文件中配置一下该类，才能自动注入Applicati onContext对象 ** <bean
 * class="util.HibernateConfiguration Util"/> * /
 */
@Repository("hibernateConfigurationUtil")
